...
namespace StRttmy.Common
{
    ...
    public class MachineInfo
    {
        ...
        public string GetMacByNetworkInterface()
        {
            ...
            if (macs[0].IndexOf(':') > -1)
            {
                var MacArr1 = macs[0].Split(':');
                foreach (string m in MacArr1)
                {
                    MacStr += m.ToLower();
                }
            }
            ...
            var MacArr = macs[0].ToArray();
            foreach (char m in MacArr)
            {
                MacStr += m.ToString().ToLower();
            }
            return MacStr;
        }
        ...
        public string example()
        {
            ...
            //Info += "\r\n" + info.GetMacAddress(LocalIp[0]);//通过获取的第一个IP来获取Mac地址
            Info = info.GetMacAddress(LocalIp[0]);
            //Info += "\r\n";
        ...
        public string GetDiskSerialNumber()
        {
            ...
            foreach (ManagementObject mo in moc)
            {
                ...
                break;//这解决有多个物理盘时产生的问题，只取第一个物理硬盘  
            }
            return HDid;
